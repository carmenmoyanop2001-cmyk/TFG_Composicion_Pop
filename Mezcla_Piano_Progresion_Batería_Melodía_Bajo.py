...
def definir_estructura(bpm, metrica, segundos):
    ...
    if dur_compas <= 0:
        raise ValueError("Duración de compás inválida")
    ...
    for i in range(num_compases):
        if i < 2:
            mapa.append("INTRO")
        elif i < num_compases // 2:
            mapa.append("VERSO")
        elif i < num_compases - 2:
            mapa.append("ESTRIBILLO")
        else:
            mapa.append("FINAL")
...
def produccion_final_maestra():
    ...
    # Recogemos num_compases correctamente
    ...
    # Inicializar objeto MIDI Multicanal (4 pistas)
    ...
    try:
        ...
        else: m_raw = np.zeros(largo_objetivo)
        ...
        return final_audio, bpm_real, animo_in, midi_maestro
...
if __name__ == "__main__":
    resultado, bpm, emo, midi_final = produccion_final_maestra()
    ...
    if resultado is not None:
        ...
        # Guardar MIDI Multicanal
        with open(f"{base_name}.mid", "wb") as f_mid:
            midi_final.writeFile(f_mid)
        
        print("\n" + "="*60)
        print(f" ÉXITO: Generados '{base_name}.wav' y '{base_name}.mid'")
        print("="*60)