...
def produccion_final_maestra(
    ...
    mapa_secciones, pulsos_p_compas, num_compases = definir_estructura(
        bpm_in, metrica_in, segundos_in
    )
    largo_objetivo = int(segundos_in * SR)

    # LETRA
    letra_final = Generador_de_Letras.generar_letra_completa(
        mapa_secciones, animo_in
    )

    midi_maestro = MIDIFile(4)
    for i in range(4):
        midi_maestro.addTempo(i, 0, bpm_in)
    ...
    try:
        ...
        if usa_pia:
            for acorde, inicio, dur in midi_p:
                for f in acorde:
                    ...
                    if n:
                        midi_maestro.addNote(0, 0, n, inicio, dur, 60)
        else:
            p_raw = np.zeros(largo_objetivo)
        ...
        if usa_baj:
            ...
            for n_baj, inicio, dur in midi_ba:
                if n_baj:
                    midi_maestro.addNote(1, 0, n_baj, inicio, dur, 80)
        else:
            ba_raw = np.zeros(largo_objetivo)
        ...
        # MEZCLA
        mezcla = (
            forzar_largo(m_raw, largo_objetivo) * 0.45 +
